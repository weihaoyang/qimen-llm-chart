import type { DecisionDNARecord } from "../types";
...
export const sessionApi = {
  ...
  saveModule: (battleId:string, moduleId:string, state:unknown, consent:unknown = {}, options:{ idempotencyKey?:string; expectedVersion?:number } = {}) => request<{ state: unknown }>(`/api/battles/${battleId}/modules/${moduleId}`, { method:"PUT", headers: options.idempotencyKey ? { 'Idempotency-Key': options.idempotencyKey } : undefined, body:JSON.stringify({ state, consent, ...(options.expectedVersion === undefined ? {} : { expectedVersion: options.expectedVersion }) }) }),
  ...
  advice: (battleId:string) => request<{ advice:Array<Record<string, unknown>> }>(`/api/battles/${battleId}/advice`),
  createAdvice: (battleId:string, input:{ targetType:"battle"|"fact"|"junction"|"move"|"commitment"|"review"; targetId?:string|null; opinion:string; rationale:string; uncertainty:string; idempotencyKey:string }) => request<{ advice:Record<string, unknown> }>(`/api/battles/${battleId}/advice`, { method:"POST", headers:{ 'Idempotency-Key': input.idempotencyKey }, body:JSON.stringify(input) }),
  ...
  profile: () => request<{ profile: { profile: Record<string, unknown> } | null; decisionDna?: DecisionDNARecord[]; battleStats?: { totalSimulations:number; singularitySuccessRate:number }; archonProgress:{ reviewedBattles:number; committedBattles:number; collaborationBattles:number; archiveUnlocks:number; score:number; rankTitle:string; seals:number; privileges:{ precognition:boolean; archiveAnnotation:boolean; realityProposal:boolean } } }>(`/api/battles/profile`),
