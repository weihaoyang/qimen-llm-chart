...
type CollaboratorRow = { id:string; battle_id:string; subject_type:string; subject_id:string; role:CollaboratorRole; status:Collaborator["status"]; permissions_json:Json; created_at:Date; updated_at:Date };
const mapCollaborator = (r:CollaboratorRow): Collaborator => ({ id:r.id,battleId:r.battle_id,subjectType:r.subject_type,subjectId:r.subject_id,role:r.role,status:r.status,permissions:record(r.permissions_json),createdAt:r.created_at.toISOString(),updatedAt:r.updated_at.toISOString() });
export const listCollaborators = async (subject:AccountSubject,battleId:string) => { if(!await owned(subject,battleId)) return null; const result=await query<CollaboratorRow>(`SELECT id,battle_id,subject_type,subject_id,role,status,permissions_json,created_at,updated_at FROM battle_collaborators WHERE battle_id=$1 ORDER BY created_at`,[battleId]); return result.rows.map(mapCollaborator); };
export const upsertCollaborator = async (subject:AccountSubject,battleId:string,input:{subjectType:string;subjectId:string;role:CollaboratorRole;permissions:Json}) => withTransaction(async(client)=>{const check=await client.query(`SELECT id FROM battle_cases WHERE id=$1 AND platform_subject_type=$2 AND platform_subject_id=$3 FOR UPDATE`,[battleId,...owner(subject)]);if(!check.rowCount)return null;const row=await client.query<CollaboratorRow>(`INSERT INTO battle_collaborators(id,battle_id,subject_type,subject_id,role,status,permissions_json,invited_by_type,invited_by_id) VALUES($1,$2,$3,$4,$5,'invited',$6::jsonb,$7,$8) ON CONFLICT(battle_id,subject_type,subject_id) DO UPDATE SET role=EXCLUDED.role,status='invited',permissions_json=EXCLUDED.permissions_json,updated_at=now() RETURNING id,battle_id,subject_type,subject_id,role,status,permissions_json,created_at,updated_at`,[randomUUID(),battleId,input.subjectType,input.subjectId,input.role,JSON.stringify(input.permissions),subject.subjectType,subject.subjectId]);return mapCollaborator(row.rows[0]);});
