...
export const listCollaborators = async (subject:AccountSubject,battleId:string) => {
  ...
  if (!await accessible(subject, battleId)) return null;
  const result = await query<CollaboratorRow>(`SELECT id,battle_id,subject_type,subject_id,role,status,permissions_json,created_at,updated_at FROM battle_collaborators WHERE battle_id=$1 ORDER BY created_at`,[battleId]);
