...
export function DivinationPanel(props: Props) {
  ...
  if (kind === "astro") {
    return <section className="divination-panel divination-panel--astro" aria-label="星盘">
      ...
      <header className="divination-panel__header"><div><p className="divination-panel__kicker">ASTRO / NATAL CHART</p><h2>星盘</h2><p className="divination-panel__subhead">把出生时刻转换成可检视的天文坐标。</p></div><div className="astro-orbit" aria-hidden="true"><i /><i /><i /><b /></div></header>
  ...
  if (kind === "human-design") {
    return <section className="divination-panel divination-panel--human" aria-label="人类图">
      ...
      <header className="divination-panel__header"><div><p className="divination-panel__kicker">HUMAN DESIGN / ACTIVATION MAP</p><h2>人类图</h2><p className="divination-panel__subhead">看见人格与设计两侧的激活信号，不替你下人格结论。</p></div><div className="hd-glyph" aria-hidden="true"><span /><span /><span /><span /><b /></div></header>
      <div className="divination-panel__rule" />
      <div className="hd-summary"><div><small>TYPE</small><strong>待独立推导</strong><span>不作猜测</span></div><div><small>PRECISION</small><strong>{value.precision?.gate ?? "—"}</strong><span>gate / line 可用</span></div><div><small>ENGINE</small><strong>13 × 2</strong><span>personality / design</span></div></div>
  ...
  return <section className="divination-panel divination-panel--tarot" aria-label="塔罗牌">
    ...
    <header className="divination-panel__header"><div><p className="divination-panel__kicker">TAROT / REFLECTION DRAW</p><h2>塔罗牌</h2><p className="divination-panel__subhead">把问题摊开，给下一步留出可验证的空间。</p></div>{props.onRedraw ? <button type="button" className="divination-panel__action" onClick={props.onRedraw}><span>↻</span>重新抽牌</button> : null}</header>
    <div className="divination-panel__rule" />
    <div className="tarot-spread">{value.cards.map((card, index) => <article className={"tarot-card " + (card.orientation === "逆位" ? "is-reversed" : "")} key={card.id} style={{ "--item-index": index } as React.CSSProperties}><div className="tarot-card__top"><small>0{index + 1} / {["CURRENT", "RESISTANCE", "NEXT"][index]}</small><em>{String(card.number).padStart(2, "0")}</em></div><div className="tarot-card__sigil" aria-hidden="true">{index === 0 ? "✦" : index === 1 ? "◈" : "✳"}</div><strong>{card.name}</strong><span>{card.orientation} <i /> {card.keyword}</span><p>{card.meaning}</p><div className="tarot-card__footer">{card.arcana === "major" ? "MAJOR ARCANA" : "MINOR ARCANA"}<b>↗</b></div></article>)}</div>
