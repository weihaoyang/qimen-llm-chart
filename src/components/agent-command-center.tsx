  ...
  return <main className="agent-command" aria-label="胜天半子 Agent 决策控制室">
    ...
    <div className="agent-command__grid">
      ...
      <aside className="agent-command__model">
        ...
        <DecisionTreePanel life={life} relationshipScales={relationshipScales} embedded savedSnapshot={savedTree?.snapshot} savedVersion={savedTree?.version} onSave={canPersist ? saveDecisionTree : undefined} saveLabel={treeSaving ? "正在存档…" : savedTree ? "保存当前证据为新版本" : "保存这棵树"} />
