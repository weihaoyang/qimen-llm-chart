...
export function AgentCommandCenter({ mode, onModeChange, inspector, life, relationshipScales, question, conversationCount, canPersist, evidenceText, conversation, accessToken }: AgentCommandCenterProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/agent/cases", { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" })
      ...
      .then((data) => { if (!cancelled) { setCases(data.cases); setActiveCaseId((current) => current || data.cases[0]?.id || ""); } })
  ...
  const createWorkspace = async (): Promise<string | null> => {
    ...
    setCases((current) => [data.case!, ...current]); setActiveCaseId(data.case.id); setSavedTurnCount(0); setPersistenceStatus("工作区已写入服务器");
    return data.case.id;
  };
  ...
  return <main className="agent-command" aria-label="胜天半子 Agent 决策控制室">
    ...
    <div className="agent-command__grid">
      <aside className="agent-command__cases">
        ...
        <article className="agent-command__case is-current"><small>{activeCaseId ? "服务器工作区" : "当前临时工作区"}</small><strong>{issueTitle}</strong><p><CalendarClock size={13} /> 尚未设置决策期限</p></article>
        {(canPersist ? cases : []).filter((item) => item.id !== activeCaseId).slice(0,4).map((item) => <button type="button" className="agent-command__case agent-command__case-button" key={item.id} onClick={() => { setActiveCaseId(item.id); setSavedTurnCount(0); setPersistenceStatus(`已切换：${item.title}`); }}><small>{new Date(item.updatedAt).toLocaleDateString("zh-CN")}</small><strong>{item.title}</strong></button>)}
