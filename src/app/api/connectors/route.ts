...
export async function GET(request: Request) {
  try { return noStore({ connectors: await listConnectors(await requireAccountSubject(request)) }); }
  catch (error) { return errorResponse(error, "读取连接状态失败。"); }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (typeof provider !== "string" || !CONNECTOR_PROVIDERS.includes(provider as never) || (action !== "authorize" && action !== "revoke")) return noStore({ error: "连接器操作无效。" }, { status: 400 });
    ...
    return noStore({ connector }, { status: 200 });
  } catch (error) { return errorResponse(error, "更新连接状态失败。"); }
}
