...
export async function GET(request: Request, context: Context) {
  try { const id = (await context.params).id; if (!isAgentWorkspaceId(id)) return NextResponse.json({ error: "议题标识无效。" }, { status: 400 }); const item = await getCase(await requireAccountSubject(request), id); return item ? NextResponse.json({ case: item }) : NextResponse.json({ error: "议题不存在。" }, { status: 404 }); }
...
export async function DELETE(request: Request, context: Context) {
  try { const id = (await context.params).id; if (!isAgentWorkspaceId(id)) return NextResponse.json({ error: "议题标识无效。" }, { status: 400 }); const deleted = await deleteCase(await requireAccountSubject(request), id); return deleted ? new NextResponse(null, { status: 204 }) : NextResponse.json({ error: "议题不存在。" }, { status: 404 }); }
