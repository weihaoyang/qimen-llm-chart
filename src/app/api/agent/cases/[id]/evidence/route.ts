...
export async function GET(request: Request, context: Context) {
  try {
    ...
    if (!isAgentWorkspaceId(id)) return NextResponse.json({ error: "议题标识无效。" }, { status: 400 });
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (!isAgentWorkbenchMode(body?.mode) || typeof body?.sourceText !== "string" || !body.sourceText.trim() || new TextEncoder().encode(body.sourceText).length > 250000 || body.structuredJson === undefined || JSON.stringify(body.structuredJson).length > 300000) return NextResponse.json({ error: "证据快照无效或过大。" }, { status: 400 });
