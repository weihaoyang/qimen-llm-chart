...
export async function GET(request: Request, context: Context) {
  try { const id = (await context.params).id; if (!validId(id)) return NextResponse.json({ error: "议题标识无效。" }, { status: 400 }); return NextResponse.json({ reviews: await listReviews(await requireAccountSubject(request), id) }); }
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (body?.branchId !== undefined && body.branchId !== null && (typeof body.branchId !== "string" || !validId(body.branchId))) return NextResponse.json({ error: "分支标识无效。" }, { status: 400 });
