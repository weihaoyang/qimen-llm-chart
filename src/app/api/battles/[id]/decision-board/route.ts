...
export async function POST(request: Request, context: Context) {
  try {
    ...
    if (action === "comment") {
      ...
      if (!targetType || !targetTypes.has(targetType) || !targetTitle || !content) return noStore({ error:"评论字段无效。" }, { status:400 });
      ...
      if (!commentId) return noStore({ error:"评论标识无效。" }, { status:400 });
      ...
      if (!strategyName || !coreThesis || !suggestedAction || !pros || !cons || typeof estimatedSurvivalProb !== "number" || !Number.isFinite(estimatedSurvivalProb) || estimatedSurvivalProb < 0 || estimatedSurvivalProb > 100) return noStore({ error:"并行策略字段无效。" }, { status:400 });
      ...
      if (typeof body.enabled !== "boolean") return noStore({ error:"脱敏开关字段无效。" }, { status:400 });
      mutation = { type:"redaction", enabled:body.enabled, idempotencyKey };
    } else return noStore({ error:"决策委员会操作无效。" }, { status:400 });
    const result = await mutateDecisionBoard(await requireAccountSubject(request), id, mutation);
    if (result === null) return noStore({ error:"战局不存在或无协作访问权限。" }, { status:404 });
    if (result === "forbidden") return noStore({ error:"当前协作角色无权执行该委员会操作。" }, { status:403 });
    return noStore({ state:result });
  } catch (error) {
    return errorResponse(error, "更新决策委员会失败。");
  }
}
