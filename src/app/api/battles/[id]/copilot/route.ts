...
export async function POST(request:Request,context:Context){
  ...
  try{
    ...
    if(!question||body?.history!==undefined&&!isHistory(body.history))return noStore({error:"问题或对话上下文无效。"},{status:400});
    ...
    if(!gate.allowed)return noStore({error:gate.message||"当前账户还没有推演官权益。",reasonCode:gate.reason_code||"entitlement_gate_blocked",gate},{status:402});
    ...
    reservationId=reservation.reservation_id; if(!reservationId)throw new UserFacingError("无法预留本次推演，请稍后重试。");
    ...
    return errorResponse(error, "推演官暂时不可用。", 500);
  }
}
