...
export async function PATCH(request: Request, context: Context) {
  try {
    ...
    if (!source) return noStore({ error: "策略来源状态必须是 JSON 对象。" }, { status: 400 });
    const updated = await updateDraftMoveSource(await requireAccountSubject(request), id, moveId, source);
    return updated ? noStore({ move: updated }) : noStore({ error: "只有当前账户的草案策略可以更新。" }, { status: 409 });
  } catch (error) {
    return errorResponse(error, "策略状态保存失败。");
  }
}
